...
if __name__ == '__main__':
    ...
    for epoch_idx in range(args.epochs):
        ...
        cluster_w_fedavg, cluster_net, optimizer_cn, cluster_size = [], [], [], []
        init_w_gen = copy.deepcopy(generator.state_dict())
        latest_update_clui, latest_update = -1, -1
        for clui, clu in enumerate(cluster_last_update):
            ...
            if clu > latest_update:
                latest_update = clu
                latest_update_clui = clui

        for clui, clu in enumerate(cluster_last_update):
            if clui == latest_update_clui:
                continue
            optimizer_cn = torch.optim.SGD(cluster_net[clui].parameters(), lr=args.lr_S,weight_decay=args.weight_decay, momentum=0.0)
            generator.load_state_dict(copy.deepcopy(init_w_gen))
            DFAN_single(args, cluster_net[latest_update_clui], cluster_net[clui], generator, (optimizer_cn, optimizer_gen), epoch_idx, reg=True)
        generator.load_state_dict(copy.deepcopy(init_w_gen))

        w_clusters = []
        for cn in cluster_net:
            w_clusters.append(copy.deepcopy(cn.state_dict()))
        net_glob.load_state_dict(copy.deepcopy(FedAvg(w_clusters)))
