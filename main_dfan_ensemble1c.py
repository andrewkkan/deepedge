...
if __name__ == '__main__':
    ...
    local_user = []
    local_user_num_updates = {}
    local_user_num_updates[-1] = []
    ...
    for epoch_idx in range(args.epochs):
        ...
        for user_idx in idxs_users:
            if args.async_s2d == 1:  # async mode 1 updates after FedAvg (see lines below FedAvg)
                ...
                local_user[user_idx].weight_update(net=copy.deepcopy(net_glob).to(args.device))
                local_user_num_updates[last_update[user_idx]].remove(user_idx)
                local_user_num_updates[epoch_idx].append(user_idx)
        ...
        for clui, clu in enumerate(cluster_last_update):
            ...
            if cluster_local.size > largest_cluster:
                ...
                largest_cluster_local = cluster_local

        if largest_cluster == 1:
            w_fedavg_largest_cluster = copy.deepcopy(net_locals[int(largest_cluster_local[0])].state_dict())
        else:
            w_locals = []
            for cli, cl in enumerate(largest_cluster_local):
                w_locals.append(copy.deepcopy(net_locals[int(cl)].state_dict()))
            w_fedavg_largest_cluster = FedAvg(w_locals)
        ...
        net_glob.load_state_dict(copy.deepcopy(w_fedavg_largest_cluster))
        # for ii in range(3):
