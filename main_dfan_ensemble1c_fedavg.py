...
if __name__ == '__main__':
    ...
    for epoch_idx in range(args.epochs):
        ...
        cluster_net_copy = copy.deepcopy(cluster_net)
        for clui, clu in enumerate(cluster_last_update):
            optimizer_cn.append(torch.optim.SGD(cluster_net[clui].parameters(), lr=args.lr_S,weight_decay=args.weight_decay, momentum=0.0))
            generator.load_state_dict(copy.deepcopy(init_w_gen))
            DFAN_ensemble(args, cluster_net_copy, cluster_net[clui], generator, (optimizer_cn[clui], optimizer_gen), epoch_idx)
        generator.load_state_dict(copy.deepcopy(init_w_gen))
