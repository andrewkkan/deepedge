...
if __name__ == '__main__':
    ...
    for epoch_idx in range(args.epochs):
        ...
        cluster_w_ref, cluster_net, cluster_w_gen, optimizer_cn, cluster_size = [], [], [], [], []
        ...
        for iii in range(3):
            ...
            for clui, clu in enumerate(cluster_last_update):        
                ...
                net_ensemble[0].load_state_dict(copy.deepcopy(cluster_w_ref[clui]))
                net_ensemble[1].load_state_dict(copy.deepcopy(cluster_net[clui].state_dict()))
                generator.load_state_dict(cluster_w_gen[clui])
                DFAN_ensemble(args, net_ensemble, cluster_net[clui], generator, (optimizer_cn[clui], optimizer_gen), epoch_idx)
                cluster_w_gen.append(copy.deepcopy(generator.state_dict()))
                cluster_w_ref[clui] = copy.deepcopy(cluster_net[clui].state_dict())

            net_ensemble = cluster_net
        ...
        # w_ref = copy.deepcopy(w_fedavg)
        # # for ii in range(3):
        # DFAN_ensemble(args, net_locals, net_glob, generator, (optimizer_glob, optimizer_gen), epoch_idx)

        # net_ensemble = []
        # net_ensemble.append(MLP(1024, 200, 10).to(args.device).eval())
        # net_ensemble.append(MLP(1024, 200, 10).to(args.device).eval())
        # net_ensemble[0].load_state_dict(w_ref)
        # net_ensemble[1].load_state_dict(copy.deepcopy(net_glob.state_dict()))
        # DFAN_ensemble(args, net_ensemble, net_glob, generator, (optimizer_glob, optimizer_gen), epoch_idx)
        # w_ref = copy.deepcopy(net_glob.state_dict())
