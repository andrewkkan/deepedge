...
if __name__ == '__main__':
    ...
    for epoch_idx in range(args.epochs):
        net_locals, net_last_update, loss_locals, acc_locals, acc_locals_on_local = [], [], [], [], []
        ...
        for iu_idx, user_idx in enumerate(idxs_users):
            if args.async_s2d == 1:  # async mode 1 updates after FedAvg (see lines below FedAvg)
                ...
                net_last_update.append(copy.deepcopy(local_user[user_idx].last_net))
        ...
        if args.fedavg_update:
            w_last_update = []
            for nlu in net_last_update:
                w_last_update.append(nlu.state_dict())
            w_fedavg = FedAvgUpdate(w_locals, w_last_update, net_glob.state_dict())
        else:
