...
class LocalClient_HypGrad(object):
    def __init__(self, args, net, dataset=None, idxs=None, user_idx=0):
        ...
        self.lr_local_adjusts = self.num_local_steps // self.lr_local_interval
    ...
    def train(self) -> Tuple[Dict[str, Union[torch.Tensor, List[float]]], float, float]:
        ...
        for lr_adjust_idx in range(self.lr_local_adjusts):
            with torch.no_grad():
            ...
            lr_local_grad: float = ((flat_delts * self.desc_glob).sum()).item() / float(self.lr_local_interval) # dot prouduct normalized 
        ...
        return {'delt_w': flat_delts, 'lr_local_grad': self.active_state['lr_local_grad'], }, mean_loss, mean_accuracy


    def sync_update(self, net: Optional[NNet]=None, desc_glob: Optional[torch.Tensor]=None, lr_local: Optional[List[float]]=None):
