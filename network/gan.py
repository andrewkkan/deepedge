...
class GeneratorA(nn.Module):
    def __init__(self, nz=100, ngf=64, nc=1, img_size=(1,32,32)):
        super(GeneratorA, self).__init__()

        # (# channels, img dim 0, img dim 1)
        # See scale_factor values in interpolate functions below
        self.init_size = (img_size[0], img_size[1]//4, img_size[2]//4)
        self.l1 = nn.Sequential(nn.Linear(nz,
                                          ngf*2*self.init_size[0]*self.init_size[1]*self.init_size[2]))
    ...
    def forward(self, z):
        ...
        out = out.view(out.shape[0], -1, self.init_size[1], self.init_size[2])
